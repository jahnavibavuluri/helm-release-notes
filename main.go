...
func main() {
	...
	oldVars, err := parseVariablesTF(oldPath)
	if err != nil {
		fmt.Printf("Error parsing old variables.tf: %v\n", err)
		return
	}

	newVars, err := parseVariablesTF(newPath)
	if err != nil {
	...
	evalCtxVars := buildEvalContext(oldVars, map[string]cty.Value{})

	// Step 2: Parse and evaluate locals using the context with variables
	oldLocals, _ := parseLocalsTF(filepath.Join(os.Args[1], "main.tf"), evalCtxVars)

	// Step 3: Build the final context with both variables and evaluated locals
	evalCtx := buildEvalContext(oldVars, oldLocals)

	oldResources, oldHelmReleases, err := parseMainTF(filepath.Join(os.Args[1], "main.tf"), evalCtx)
	if err != nil {
		fmt.Printf("Error parsing old main.tf: %v\n", err)
		return
	}

	evalCtxVars = buildEvalContext(newVars, map[string]cty.Value{})
	newLocals, _ := parseLocalsTF(filepath.Join(os.Args[2], "main.tf"), evalCtxVars)
	evalCtx = buildEvalContext(newVars, newLocals)
	newResources, newHelmReleases, err := parseMainTF(filepath.Join(os.Args[2], "main.tf"), evalCtx)
	if err != nil {
	...
	diffResources(oldResources, newResources)

	fmt.Println("Helm Release Changes:")
	diffHelmReleases(oldHelmReleases, newHelmReleases)
